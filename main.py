...
def repeating_items(): 
    ...
    def repeat():
        ...
        for tuple in repeating_items_list.repeating_items_list_list:
            ...
            if tuple[0].reference.finished:
                #check if there is a cap
                if not tuple[0].cap:
                    #check if the amount of times repeated is not equal to or above the cap
                    if not tuple[0].times == tuple[0].cap or not tuple[0].times > tuple[0].cap:
                        #new name
                        name = f"{tuple[0].name[:-1]}{int(tuple[0].name[-1:]) + 1}"
                        #update repeating items

                        #update repeating_items.py
                        with open("repeating_items.py","a") as repeating:
                            
                            repeating.write(f"\n{tuple[0].name} = construction_repeat(\"{tuple[0].name}\", {tuple[0].times + 1}, {tuple[0].times * tuple[0].multiplier}, \"{tuple[0].interval}\", {tuple[0].multiplier}, {tuple[0].cap}, items.{name}, )")
                        #update items

                        #update items.py
                        with open("items.py","a") as items:
                            items.write(f"{name} = Construction(\"{name}\", \"{tuple[0].reference.total_time}\", \"{int(tuple[0].reference.total_time) - read_time_file()}\",\"{int(tuple[0].reference.total_time) + int(tuple[0].interval)}\", False)\n")
